...
from pypower_pych.idx_brch import F_BUS, T_BUS, RATE_A

# 所有的函数都需要转换成torch的
from pypower_pych.makeSbus import makeSbus
from pypower_pych.dSbus_dV import dSbus_dV
from pypower_pych.dIbr_dV import dIbr_dV
from pypower_pych.dSbr_dV import dSbr_dV
from pypower_pych.dAbr_dV import dAbr_dV
...
def opf_consfcn(x, pack_para, Ybus, Yf, Yt, ppopt, il=None, *args):
    ...
    neg_Cg = torch.sparse_coo_tensor(torch.stack([gen[:, GEN_BUS].int(), arange(ng)], dim=0), values=-ones(ng),  size=(nb, ng)).to_sparse(layout=torch.sparse_csr).to_dense()
