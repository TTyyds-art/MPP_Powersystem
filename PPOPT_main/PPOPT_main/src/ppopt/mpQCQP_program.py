...
class MPQCQP_Program():
    ...
    def __init__(self, net=None, lb_loads=0.8, ub_loads=1.2):
        ...
        self.b_t = np.vstack([np.eye(n_load * 2) * ub_loads, -np.eye(n_load * 2) * lb_loads]) @ r_[Pd[idx_load], Qd[idx_load]]  # 生成b_t=[ub*I, -lb*I].T*loads_0

        self.A, self.b = self.cal_ieq_params()  # 生成A, b矩阵, 作为不等式约束的参数
        self.F = zeros((self.A.shape[0], n_load*2))  # 生成F矩阵, Ax-F*loads <= b
