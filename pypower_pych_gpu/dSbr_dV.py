...
def dSbr_dV(branch, Yf, Yt, V):
    ...
    if issparse(Yf):
        ...
        temp1       = asmatrix( zeros((nl, nb), complex, device=device) )
        temp2       = asmatrix( zeros((nl, nb), complex, device=device) )
        temp3       = asmatrix( zeros((nl, nb), complex, device=device) )
        temp4       = asmatrix( zeros((nl, nb), complex, device=device) )
        for i in range(nl):
            fi, ti = f[i], t[i]
