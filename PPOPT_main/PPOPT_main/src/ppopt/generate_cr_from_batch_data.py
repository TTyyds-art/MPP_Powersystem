...
loaded_dataset = torch.load('dataset_test_118_bus_torch/dataset_20230901_015110_N10000.pth', map_location=device)  # 加载数据集
ineq_keep_del = [375, 380, 388, 396, 408, 437, 459, 544, 550, 552, 560, 566, 567, 573, 576, 577, 578, 580, 585, 589, 590, 592, 715, 716, 717, 718, 721, 722, 723, 724, 727, 729, 730, 731, 732, 733, 737, 738, 741, 744, 745, 746, 747, 748, 749, 751, 754, 755, 756, 757, 760, 761, 762, 763, 765, 766, 767, 779, 784, 797, 590]
...
# 将file_path加入当前时间
file_path = 'solution_118bus_instance_{}.joblib'.format(time.strftime("%Y%m%d_%H%M%S", time.localtime()))
# file_path = 'solution_instance_10000.joblib'
...
#         idx_act = find(
#             mu_ > 1e-3).tolist()  # 修改成了 1e-3, 因为对互补性的阈值是1e-6，阈值=z*mu/(1+max(x)) => z*mu<5e-6;mu>1e-3能够保证z<1e-3
#         cri_region = gen_cr_from_active_set_torch_add(new_QCQP, active_set=idx_act,
#                                                       keep_ineq_indices=ineq_keep_del)  # 生成当前的critical region
