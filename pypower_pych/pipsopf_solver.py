...
from pypower_pych.csr_real_imag import csr_imag
...
from pypower_pych.idx_cost import MODEL, PW_LINEAR, NCOST
from pypower_pych.idx_gen import GEN_BUS, PG, QG, VG, MU_PMAX, MU_PMIN, MU_QMAX, MU_QMIN
...
from pypower_pych.opf_hessfcn import opf_hessfcn
from pypower_pych.pips import pips
...
def pipsopf_solver_torch(om, ppopt, x0_init=None, out_opt=None):
    ...
    baseMVA, bus, gen, branch, gencost = \
        tensor(ppc["baseMVA"]), tensor(ppc["bus"]), tensor(ppc["gen"]), \
                            tensor(ppc["branch"]), tensor(ppc["gencost"])
    ...
    x0, xmin, xmax = tensor(x0), tensor(xmin), tensor(xmax)
    ...
    muSf = zeros(nl, dtype=torch.complex128)
    muSt = zeros(nl, dtype=torch.complex128)
    if len(il) > 0:
        muSf[il] = \
    ...
    nl_mu_l = zeros(nlnN)
    nl_mu_u = cat([zeros(2*nb), muSf, muSt])
