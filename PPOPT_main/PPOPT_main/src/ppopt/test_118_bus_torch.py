...
def main():
    ...
    if len(dataset) == 0:
        print("The dataset is empty. Exiting the program.")
    ...
    filename = f"dataset_{current_time}_N{Add_tot}.pth"
    full_path = os.path.join(folder_name, filename)
    torch.save(dataset, full_path)  # 保存数据集
    save_dict_to_excel(idx_act_dict, filename=f"idx_act_dict_{current_time}_N{Add_tot}.xlsx", path=folder_name)
...
def add_data_per_type(all_data_lists, idx_act, new_QCQP, pq_load):
    ...
    data_dict = {
        'pd_load': torch.tensor(pq_load).to(device),
        'idx_act': idx_act,
