...
def pips(f_fcn, x0=None, A=None, l=None, u=None, xmin=None, xmax=None,
    ...
    if l is None or len(l) == 0: l = -Inf * ones(nA, dtype=torch.float64, device=device)
    if u is None or len(u) == 0: u =  Inf * ones(nA, dtype=torch.float64, device=device)
    if xmin is None or len(xmin) == 0: xmin = -Inf * ones(x0.shape[0], dtype=torch.float64, device=device)
    if xmax is None or len(xmax) == 0: xmax =  Inf * ones(x0.shape[0], dtype=torch.float64, device=device)
    if gh_fcn is None:
        nonlinear = False
        gn = tensor([])
        hn = tensor([])
    ...
    while (not converged) and (i < opt["max_it"]):
        ...
        dz = -h - z if dh is None else -h.to(torch.float32) - z - dh.T.matmul(dx)
...
#            h = hn if Ae is None else r_[hn, Ae * x - be] # eq constraints
    ...
    mu_l = zeros(nx + nA, dtype=torch.float64, device=device)
    mu_l[ieq[kl]] = -lam_lin[kl]
    mu_l[igt] = mu_lin[nlt:nlt + ngt]
    mu_l[ibx] = mu_lin[nlt + ngt + nbx:nlt + ngt + nbx + nbx]

    mu_u = zeros(nx + nA, dtype=torch.float64, device=device)
