...
from pypower_pych.torch_utils import find
from pypower_pych.torch_utils import vstack, hstack
from pypower_pych.torch_utils import sparse, issparse

from pypower_pych.csr_real_imag import csr_real, csr_imag, cr_real, cr_imag
from pypower_pych.d2AIbr_dV2 import d2AIbr_dV2
from pypower_pych.d2ASbr_dV2 import d2ASbr_dV2
from pypower_pych.d2Sbus_dV2 import d2Sbus_dV2
from pypower_pych.dIbr_dV import dIbr_dV
from pypower_pych.dSbr_dV import dSbr_dV
from pypower_pych.idx_brch import F_BUS, T_BUS
from pypower_pych.idx_cost import MODEL, POLYNOMIAL
from pypower_pych.idx_gen import PG, QG
from pypower_pych.opf_consfcn import opf_consfcn
from pypower_pych.opf_costfcn import opf_costfcn
from pypower_pych.polycost import polycost
...
def opf_hessfcn(x, lmbda, pack_para, Ybus, Yf, Yt, ppopt, il=None, cost_mult=1.0):
    ...
    i = cat([arange(vv["i1"]["Pg"], vv["iN"]["Pg"]),
...
#                  (i, i)), shape=(nxyz, nxyz))
    ...
    if issparse(N) and N.nnz > 0: # pragma: no cover
        ...
        kbar = sparse((cat([ones(len(iLT)), zeros(len(iEQ)), -ones(len(iGT))]),
                       (iND, iND)), (nw, nw)) * kk
    ...
    if ppopt['OPF_FLOW_LIM'] == 2:       ## current
        if Yf.size:
            ...
            Hfaa= Hfav= Hfva= Hfvv= Htaa= Htav= Htva= Htvv = zeros((nb,nb)).to_sparse(layout='csr')
