...
# 该程序是可行性与最优性检查交替进行的，如果可行性检查通过，则进行最优性检查，如果最优性检查不通过，则不进行可行性检查
...
pool = Pool(64)  # to 64, compare the time consumption TODO
...
# print(len(to_check))
    ...
    f = lambda x: full_process(my_QCQP, x, murder_list=murder_list, gen_children=gen_children)
