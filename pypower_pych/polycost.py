...
from pypower_pych.idx_cost import MODEL, NCOST, PW_LINEAR, COST
...
def polycost(gencost, Pg, der=0):
    ...
    ## evaluate polynomial
    if len(c) == 0:
        f = zeros(Pg.shape, device=device)
    else:
        f = c[:, :1].flatten()  ## constant term
        for k in arange(1, c.shape[1]).to(device):
            f = f + c[:, k] * Pg**k
